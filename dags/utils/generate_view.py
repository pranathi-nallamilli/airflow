...
def fields_for_data_hash(schema, exception_list):
  exception_list = [ele[0].upper() for ele in exception_list]
  header_string = ''
  for ele in schema:
    if ele[0] not in exception_list:
      header_string += ele[1]+'::VARCHAR,'
    else:
      continue
