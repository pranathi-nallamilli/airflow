...
with DAG(dag_id="data_prep_pipeline", default_args=default_args, catchup=False) as dag:
    ...
    set_iteration_variables = PythonOperator(
        task_id="set_iteration_variables",
        python_callable=gi.set_iteration_variables,
        dag=dag,
    )

    manifest_file = PythonOperator(
        task_id="manifest_file", python_callable=fp.check_file_path, dag=dag
    )

    validate_file = PythonOperator(
        task_id="validate_file", python_callable=dvv.derive_and_validate_file, dag=dag
    )
    ...
    get_schema = PythonOperator(
        task_id="get_schema", python_callable=gc.initiate_get_schema, dag=dag
    )
    ...
    data_catalog_lookup_task = PythonOperator(
        task_id="data_catalog_lookup", python_callable=dc.lookup_data_catalog, dag=dag
    )

    mapping_and_validation = PythonOperator(
        task_id="mapping_and_validation",
